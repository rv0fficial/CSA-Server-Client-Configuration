...
22. Follow the procedure from DNS-Guide 01 in the lab sheet to change the Fedora machine's hostname (Step 1, 2, 3).
...
23. After changing the machine name, add DNS server details to the client's `resolv.conf` file:
    ...
    The above line should be reimplemented after a reboot of the fedora. (VMnet2 should be only up and running)
    
25. Restart its network services:
    ```bash
    service network restart
    ```

26. Test DNS Server from the client side using the commands mentioned in Step 05.
